...
def get_length_period(velocity: float) -> (float, float):
    ...
    min_hz = 0.5
    max_hz = 3.0
    preferred_step_length = 0.050  #FIXME constany
...
def walking():
    ...
    step_height = 0.020 #FIXME This should not be a constant (025)
    ...
    while True:
        ...
        if not standing:
            ...
            for leg_id in LegId:
                ...
                """
                adjusted_step_length = step_length
                rotation_rate = turnrate  #FIXME  hack for testig

                # Compute step_air_fraction if required
                if static_step_air_fraction > 0.0:
                    step_air_fraction = static_step_air_fraction
                else:
                    step_air_fraction = min(
                                            0.4 / step_period,  #FIXME constant
                                            0.20 * step_period) #FIXME constant
                ...
                if leg_phase < step_air_fraction:
                    ...
                    foot_y = get_delta_y(air_fraction_completed,  # unitless
                                         step_period,             # seconds
                                         adjusted_step_length,    # meters
                                         rotation_rate)           # radians per second
                                         
                    # Is this a front or real leg?
                    if leg_id in (LegId.LR, LegId.RR):
                        # This is a rear leg, flip the direction.
                        foot_y = -foot_y
                    ...
                    Lp[leg_id.value, :3] = [foot_x + x_neutral,
                                        foot_z - body_center_up,
                                        foot_y + y_neutral]
                    # DEBUGGING ONLY
                    ...
                    ground_fraction_completed = \
                        (leg_phase - step_air_fraction) / (1.0 - step_air_fraction)
                    assert 0.0 <= ground_fraction_completed <= 1.0
                    
                    foot_y = get_delta_y(ground_fraction_completed,
                                         step_period,             # seconds
                                         adjusted_step_length,    # meters
                                         rotation_rate)           # radians per second
                                         
                    # Is this a front or real leg?
                    if leg_id in (LegId.LF, LegId.RF):
                        # This is a front leg, flip the direction.
                        foot_y = -foot_y
                    ...
                    Lp[leg_id.value, 2] = Lp_neutral[leg_id.value, 2] + foot_y
                    # DEBUGGING ONLY
